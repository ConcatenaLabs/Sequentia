...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    // chain params
    argsman.AddArg("-allowplaceholdergenesis", "SEQUENTIA: allow starting -chain=sequentia on the published PLACEHOLDER genesis (its founder key is public; controls all SEQ). For dry runs only; a real launch must regenerate the genesis with a secret key. (default: 0)", ArgsManager::ALLOW_ANY, OptionsCategory::CHAINPARAMS);
    ...
    argsman.AddArg("-seqoburl=<url>", "SEQUENTIA: the SeqDEX order-book relay this wallet reads the market from when converting staking rewards "
                   "(see setrewardautoconvert). A plain http:// URL, because the node links no TLS library; what crosses the wire is a signed public "
                   "order book, and nothing here is trusted with funds. Unset by default, and never contacted unless a wallet has conversion switched on.", ArgsManager::ALLOW_ANY, OptionsCategory::ELEMENTS);
    argsman.AddArg("-seqobtimeout=<n>", "SEQUENTIA: timeout in seconds for SeqDEX relay requests (default: 30)", ArgsManager::ALLOW_ANY, OptionsCategory::ELEMENTS);
    ...
    argsman.AddArg("-anchoravoidcontested", strprintf("SEQUENTIA: when producing a block, do not advance its Bitcoin anchor onto a parent-chain height a competing branch is currently contesting; anchor to the last uncontested (commonly-agreed) block instead, so the new block needs no reorg when the parent fork resolves. Producer-side policy only; never affects which blocks are accepted, and only backs the anchor off while a parent fork is actually live. Only used when con_bitcoin_anchor is enabled. (default: %s)", DEFAULT_ANCHOR_AVOID_CONTESTED ? "true" : "false"), ArgsManager::ALLOW_ANY, OptionsCategory::ELEMENTS);
    argsman.AddArg("-anchorcontestwindow=<n>", strprintf("SEQUENTIA: with -anchoravoidcontested, treat a competing parent-chain branch as a live contest only when its tip is within <n> blocks of the active tip (branches further behind are losing the race and ignored, so old resolved forks never lower the anchor). Only used when con_bitcoin_anchor is enabled. (default: %d)", DEFAULT_ANCHOR_CONTEST_WINDOW), ArgsManager::ALLOW_ANY, OptionsCategory::ELEMENTS);
...
bool AppInitParameterInteraction(const ArgsManager& args)
{
    ...
    // -chain=sequentia genesis assigns the entire 400M SEQ supply to a founder
    ...
    // node started on -chain=sequentia with the known placeholder genesis aborts,
    // unless the operator explicitly accepts the risk (e.g. for a dry run).
    if (args.GetChainName() == CBaseChainParams::SEQUENTIA &&
        chainparams.GetConsensus().hashGenesisBlock ==
            uint256S("0x35775fa374dd84b07bea09f89fc2fd930097957aa30f0e9358680aabdf19af5e") &&
        !args.GetBoolArg("-allowplaceholdergenesis", false)) {
        return InitError(Untranslated(
            "Refusing to start the Sequentia network (-chain=sequentia) on the "
            "PLACEHOLDER genesis block: its founder private key is published "
            "(doc/sequentia/13) and controls the entire SEQ supply. A real launch "
            "must regenerate the genesis with a secret founder key. Use -chain=test "
            "for the public playground, or -allowplaceholdergenesis for a dry run."));
    }
