...
CTxMemPoolEntry::CTxMemPoolEntry(const CTransactionRef& tx, CAmount fee, CAsset feeAsset, CValue feeValue,
    ...
    : tx{tx},
      ...
      m_modified_fee{feeValue.GetValue()},
      lockPoints{lp},
...
{
    nModFeesWithDescendants += CValue(fee_diff);
    nModFeesWithAncestors += CValue(fee_diff);
...
void CTxMemPoolEntry::UpdateFeeValue(CValue newFeeValue)
{
    ...
    m_modified_fee = SaturatingAdd(m_modified_fee, newFeeValue.GetValue() - nFeeValue.GetValue());
...
void CTxMemPool::addUnchecked(const CTxMemPoolEntry &entry, setEntries &setAncestors, bool validFeeEstimate)
{
    ...
    Assume(entry.GetFeeValue().GetValue() == entry.GetModifiedFee());
    if (delta) {
...
static TxMempoolInfo GetInfo(CTxMemPool::indexed_transaction_set::const_iterator it) {
    return TxMempoolInfo{it->GetSharedTx(), it->GetTime(), it->GetFee(), it->GetTxSize(), it->GetModifiedFee() - it->GetFee(), it->GetDiscountTxSize(), it->GetFeeValue().GetValue()};
}
