    ...
    : QWidget(parent), m_platform_style{platformStyle}
{
    ...
    // The fee actions below are unfamiliar to most users; show their tooltips
    // when hovering the menu entries so each one explains itself in place.
...
void TransactionView::setModel(WalletModel *_model)
{
    ...
    if(_model)
    {
        ...
        // Native Bitcoin is part of the one history: the wallet's rows and the
        // parent-chain rows concatenate into a single source, and the date sort
        // interleaves them chronologically. No separate table, no separate mode.
        QAbstractItemModel* history_source = _model->getTransactionTableModel();
        if (m_parent_chain_model) {
            auto* merged = new QConcatenateTablesProxyModel(this);
            merged->addSourceModel(_model->getTransactionTableModel());
            merged->addSourceModel(m_parent_chain_model);
            history_source = merged;
        }
        transactionProxyModel->setSourceModel(history_source);
...
void TransactionView::contextualMenu(const QPoint &point)
{
    ...
    // A native-Bitcoin row lives on the parent chain: the wallet-transaction actions
    // (abandon, RBF, CPFP, replace) have no Sequentia transaction to act on. Copying
...
void TransactionView::showDetails()
{
    ...
    if(!selection.isEmpty())
    {
        TransactionDescDialog *dlg = new TransactionDescDialog(selection.at(0), model);
