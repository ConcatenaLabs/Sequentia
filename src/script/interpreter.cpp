    ...
    if (witversion == 0) {
        ...
        if (stack.size() == 1) {
            ...
            if ((flags & SCRIPT_VERIFY_SIMPLICITY) && (control[0] & TAPROOT_LEAF_MASK) == TAPROOT_LEAF_TAPSIMPLICITY) {
                ...
                // See the comment on the constant in script.h. `witness.stack` is the
                // whole stack including any annex, which is deliberate: the annex is
                // committed to by Simplicity's sigAllHash (via txHash -> inputsHash ->
                // inputAnnexesHash), so budget bought with annex bytes cannot be
                // stripped by a third party without invalidating the spend.
                const int64_t budget = std::min<int64_t>(
                    (int64_t)::GetSerializeSize(witness.stack, PROTOCOL_VERSION) * SIMPLICITY_BUDGET_PER_WITNESS_BYTE + VALIDATION_WEIGHT_OFFSET,
