...
void SetupChainParamsBaseOptions(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-posescapestallmtpheight=<n>", "SEQUENTIA: block height from which the escaping-stall parent-chain median-time-past gap is enforced. Same convention as -posexpraceheight: 0 means the rule is NOT gated (off), and a positive H enforces it from height H, exempting the history below H. A chain launched with the rule in place uses 1, i.e. active from the first block. The gate exists because the rule was added after some chains had already produced blocks that violate it, which made that history unvalidatable and the chain unsyncable from genesis. Custom/regtest chains only (the real chains pin it in code). Only used when -con_pos is set. (default: 1)", ArgsManager::ALLOW_ANY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-posblockspacing=<n>", "SEQUENTIA: minimum seconds between a block and its parent, enforced by CONSENSUS. The chain's cadence otherwise lives only in the producer (PosProducer::Step), where no validator checks it, so a modified producer can run the chain as fast as it likes -- and, collecting the fees, has every reason to. A block stamped closer than this to its parent is invalid (bad-pos-spacing). Deliberately a separate number from -posslotinterval, which is the leader time-gate unit. 0 disables the rule, which is the default here because functional tests generate blocks back to back. Custom/regtest chains only (the real chains pin it in code). Only used when -con_pos is set. (default: 0)", ArgsManager::ALLOW_ANY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-posblockspacingheight=<n>", "SEQUENTIA: block height from which -posblockspacing is enforced. Same convention as -posescapestallmtpheight. Note that the PRODUCER-side clamp follows the spacing VALUE and not this height, so a release can stop emitting too-close blocks before the consensus rule starts rejecting them -- which is the safe order on a running chain. Custom/regtest chains only (the real chains pin it in code). Only used when -con_pos is set. (default: 1)", ArgsManager::ALLOW_ANY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-posslotgateseconds=<n>", "SEQUENTIA: seconds of leader time-gate per unit of sortition score, from -posslotgateheight. The gate delays a leader by floor(score) x this after the parent, and the producer will not propose before the block spacing anyway, so every draw landing under the cadence proposes at the same moment -- the unit decides how much of the exponential race's tail the cadence absorbs. DELIBERATELY NOT -posslotinterval, which also scales the unbonding requirement. 0 keeps the historic whole-interval unit. Custom/regtest chains only (the real chains pin it in code). Only used when -con_pos is set. (default: 0)", ArgsManager::ALLOW_ANY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-posslotgateheight=<n>", "SEQUENTIA: block height from which -posslotgateseconds is the leader time-gate unit. Only meaningful under the exponential-race election: the legacy slot is a bounded rank, for which the whole-interval scale is correct. Custom/regtest chains only (the real chains pin it in code). Only used when -con_pos is set. (default: 1)", ArgsManager::ALLOW_ANY, OptionsCategory::CHAINPARAMS);

    argsman.AddArg("-supervisedassetsheight=<n>", "SEQUENTIA: block height from which supervised assets exist, i.e. from which an issuance carrying a supervision declaration derives its asset id over that declaration and so becomes freezable by its issuer. Same convention as -posexpraceheight: 0 means the rule is off, a positive H activates it from height H. Below H a declaration is inert data and the issuance derives a plain asset id, so H is a HARD FORK that every node must cross together. Custom/regtest chains only (the real chains pin it in code). (default: 1)", ArgsManager::ALLOW_ANY, OptionsCategory::CHAINPARAMS);
