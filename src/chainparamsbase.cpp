...
void SetupChainParamsBaseOptions(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-posescapestallmtpheight=<n>", "SEQUENTIA: block height from which the escaping-stall parent-chain median-time-past gap is enforced. NOTE the convention is the opposite of -posexpraceheight: here 0 means ENFORCED FROM GENESIS (the correct setting for a chain launched with the rule in place), and a positive H exempts the history below H. It exists because the rule was added after some chains had already produced blocks that violate it, which made that history unvalidatable and the chain unsyncable from genesis. Custom/regtest chains only (the real chains pin it in code). Only used when -con_pos is set. (default: 0)", ArgsManager::ALLOW_ANY, OptionsCategory::CHAINPARAMS);
