...
**Simplicity is not available on Sequentia.** The Simplicity library is vendored in the
tree but the deployment is `NEVER_ACTIVE` on every Sequentia chain
(`src/chainparams.cpp`). Worse, a Tapsimplicity leaf (version 0xbe) is treated today as
an anyone-can-spend success path. Any design that locks funds behind Simplicity covenants
on Sequentia is not a compliance feature but a fund-loss bug. The full Elements tapscript
introspection opcode set (leaf version 0xc4) is active from block 1, so covenants are
possible without Simplicity; section 5.5 explains why this spec deliberately does not use
them for USDC.
...
2. **Covenant compliance wrapper (REJECTED).** Lock every USDC.e output in an
   ...
   feasible on Sequentia (the Elements introspection set is active; Simplicity is not
   needed and, being NEVER_ACTIVE here, must not be used). Rejected because: it makes an
...
- Its central mechanism, a mandatory "Compliance Wrapper" of recursive Simplicity
  covenants with a compliance-oracle co-signature on every transfer. Simplicity is
  NEVER_ACTIVE on every Sequentia chain, and a Tapsimplicity leaf is currently an
  anyone-can-spend path here, so the proposed wrapper would not merely fail to enforce
  compliance, it would hand the wrapped funds to the first taker. Even re-based onto the
