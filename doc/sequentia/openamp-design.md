...
- **Chain follower**: tracks the chain via `sequentiad` RPC, anchor-aware; all stateful accounting keys on (block hash, Bitcoin-anchor depth) and rolls back on reorgs, per anchoring supremacy. Velocity windows and snapshot heights are measured in anchor-confirmed blocks; co-sign decisions are idempotent.
...
- **Sequentia Core** (`getopenampaccount`, `signopenamptransfer`, the GUI's OpenAMP tab): the holder's side, described for holders in [`openamp-holder.md`](openamp-holder.md). The account id and the enclave are derived offline from public keys, so a holder can check that the address they are paid at is the one the asset's own id implies rather than trusting the server to say. Signing recomputes each sighash from the transaction and refuses any that does not match what the server asked for, which the other wallets cannot do because they have no chain to check against. Core does not speak to the policy server itself: the Qt build carries no TLS backend, so the https half is left to whatever already has one.
