...
class PosAnchorFreshnessTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        #
        # Which of the two the comparator sorts higher is not fixed — B_old and
        # B_new carry the same work and the same 3 countersignatures, so the
        # order falls through to VRF score and then block hash — and only when
        # B_new sorts higher is the finality gate consulted at all. The OUTCOME
        # is the same either way, and that is what is asserted here; the gate's
        # own refusal (and its bad-fork-prior-to-pos-final log line) is asserted
        # deterministically in feature_pos_finality.py, which drives the rival
        # in over the network instead of leaving it to the sort order.
